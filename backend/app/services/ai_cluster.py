...
class GenAICluster:
    ...
    def __init__(self, api_keys: List[str]):
        ...
        if not api_keys:
            logging.warning(
                "AI Cluster initialized without keys. Failover mode active."
            )
        self.keys = api_keys
    ...
    def _sanitize_prompt(self, prompt: str) -> str:
        ...
        for pattern in malicious_patterns:
            if pattern.upper() in sanitized.upper():
                logging.critical(
                    f"SECURITY ALERT: Prompt Injection Detected: {pattern}"
                )
        ...
        for _ in range(len(self.clients)):
            try:
                ...
                response = client.models.generate_content(
                    model=model, contents=sanitized_input, config=config
                )
